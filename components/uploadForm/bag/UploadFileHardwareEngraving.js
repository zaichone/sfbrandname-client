...
function UploadFileHardwareEngraving({ taskId, clientId, setImageHardwareEngraving, images, setImages, imageFront, imageLogo, imageMadeIn, imageSerialNumber, imageHardwareEngraving, imageAdditionalImage }) {
    async function uploadFileHardwareEngraving() {
        ...
        await thisRef.put(file).then(function (snapshot) {
            snapshot.ref.getDownloadURL().then((downloadURL) => {
                ...
                setImages([imageFront, imageLogo, imageMadeIn, imageSerialNumber, imageHardwareEngraving, imageAdditionalImage]);
            });
        })
    }
    return (
        <div className="col-4 text-center mt-4">
            ...
            <SymmetricalDiv className="d-flex flex-column align-items-center justify-content-center image-box"
                onClick={() => document.getElementById("filesHardwareEngraving").click()}
                style={{ backgroundImage: `url(${imageHardwareEngraving?.imageURL})` }}
            >
                <i>Click to Add Image</i>
                <input style={{ display: "none" }} type="file" onChange={uploadFileHardwareEngraving} id="filesHardwareEngraving" name="filesHardwareEngraving[]" multiple />
            </SymmetricalDiv>
        </div>
    )
}
